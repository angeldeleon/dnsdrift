...
| **Resource exhaustion** — a hostile domain returns enormous answers | Per-RRset record caps, per-record length caps, an 8 MiB HTTP response cap, a 5 MiB config cap, a 50 MiB state cap, and a bounded worker pool |
...
| **Alert-channel injection** — a crafted DNS record injects formatting or links into Slack | Slack mrkdwn control characters are escaped in every user-influenced field |
...
- **Time-of-check/time-of-use on SSRF.** DNS can change between validation and connection. Pinning the resolved address at connect time would close this; it is not currently implemented. The guard raises the bar substantially against accidental and opportunistic SSRF without claiming to be airtight.
